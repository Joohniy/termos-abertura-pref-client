...
export const generatePDFComLetras = (nfolha, nprocesso, anoprocesso, formattedDate, nome, volencerrado, primeirafl, ultimafl, proxvolume, volaberto) => {
    const doc = new jsPDF();
    ...
    doc.setFontSize(13);
    doc.setFont("Arial");
    const stringPapelInformacao = `Papel para informação, rubricado como folha nº ${nfolha}/A`;
    ...
    doc.setFontSize(13);
    doc.setFont("Arial");
    const stringPapelInformacaoAbertura = `Papel para informação, rubricado como folha nº ${nfolha}/B`;
    ...
    const stringSecondLineAberturaAberturaAbertura = `do volume ${proxvolume} do processo ${nprocesso}/${anoprocesso} que se inicia com a folha de nº ${Number(ultimafl) + 1} que leva o`;
